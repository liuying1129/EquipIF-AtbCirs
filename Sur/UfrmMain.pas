...
VAR
  adotemp,adotemp22,adotemp33:tadoquery;
  ReqNo:integer;
  SamNo:string;
  ItemID:string;
  sValue:string;
...
begin
  ...
  adotemp.Close;
  adotemp.SQL.Clear;
  adotemp.SQL.Text:='select ReqNo,SamNo from ResultBill where format(WorDate,''YYYY-MM-DD'')='''+FormatDateTime('YYYY-MM-DD',DateTimePicker1.Date)+''' ';
  adotemp.Open;
  while not adotemp.Eof do
  begin
    GermCName:='';
    ReqNo:=adotemp.fieldbyname('ReqNo').AsInteger;
    SamNo:=adotemp.fieldbyname('SamNo').AsString;
    
    adotemp33:=tadoquery.Create(nil);
    adotemp33.Connection:=ADOConn_BS;
    adotemp33.Close;
    adotemp33.SQL.Clear;
    adotemp33.SQL.Text:='select ag.CName from ResultMIC rm inner join ATB_Germ ag on ag.GermID=rm.GermID where rm.ReqNo='+inttostr(ReqNo)+' group by ag.CName';
    adotemp33.Open;
    while not adotemp33.Eof do
    begin
      GermCName:=GermCName+adotemp33.fieldbyname('CName').AsString+' ';      
      adotemp33.Next;
    end;
    adotemp33.Free;
    GermCName:=trim(GermCName);
    
    adotemp22:=tadoquery.Create(nil);
    adotemp22.Connection:=ADOConn_BS;
    adotemp22.Close;
    adotemp22.SQL.Clear;
    adotemp22.SQL.Text:='select rm.AntiID,rm.ResShow,rm.MicExplain from ResultMIC rm where rm.ReqNo='+inttostr(ReqNo);
    adotemp22.Open;
    ReceiveItemInfo:=VarArrayCreate([0,adotemp22.RecordCount],varVariant);
    i:=0;
    while not adotemp22.Eof do
    begin
      ItemID:=adotemp22.fieldbyname('AntiID').AsString;
      sValue:=copy(adotemp22.fieldbyname('ResShow').AsString+StringofChar(' ',ValueMaxNum),1,ValueMaxNum)+adotemp22.fieldbyname('MicExplain').AsString;
      ReceiveItemInfo[i]:=VarArrayof([ItemID,sValue,'','']);
      inc(i);
      adotemp22.Next;
    end;
    adotemp22.Free;
    ReceiveItemInfo[i]:=VarArrayof(['GermCName',GermCName,'','']);
    if bRegister then
    begin
